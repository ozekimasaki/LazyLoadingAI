 ...
 *
 * Runs Claude Code programmatically against openhagen/test,
 * executing identical tasks in WITHOUT, WITH, and NATURAL modes, then
 * compares token usage and response quality side-by-side.
 *
 * Usage:
 *   npm run benchmark:validate:test                           # Full run (7 tasks)
 *   npm run benchmark:validate:test -- --dry-run              # Setup only, no API calls
 *   npm run benchmark:validate:test -- --tasks 1,4            # Run specific tasks
 *   npm run benchmark:validate:test -- --session-only --session-queries 5
 *   npm run benchmark:validate:test -- --dry-run --reindex
 *   npm run benchmark:validate:test -- --max-tool-calls 10 --format markdown
 *   npm run benchmark:validate:test -- --modes without,forced,natural --runs 10 --format compact
 *   npm run benchmark:validate:test -- --without-vs-natural   # Preset: --modes without,natural --runs 5
 *   npm run benchmark:validate:test -- --without-vs-installed # Preset: --modes without,installed --runs 5
 *   npm run benchmark:validate:test -- --runs 4
 *   npm run benchmark:validate:test -- --rescore benchmarks/results/validation-test-2026-02-15.json
 *   npm run benchmark:validate:test -- --rescore <input.json> --output <output.json>
 */
...
} from "../src/benchmarks/validation-rubric-test.ts";
...
const TEST_DIR = path.join(REPOS_DIR, "test");
...
const TASKS: TaskDef[] = [
  {
    ...
    prompt:
      "Find the middleware function that protects routes requiring authentication. What specific paths are protected, and what happens when an unauthenticated user tries to access a protected path?",
  },
  {
    id: 2,
    mode: "targeted",
    shortName: "useCodeGen",
    prompt:
      "What is the useCodeGeneration hook? Show me its implementation - what state does it manage, what does it return, and how does it route between single-model and orchestrated generation modes?",
  },
  {
    id: 3,
    mode: "targeted",
    shortName: "TaskType",
    prompt:
      "Find the TaskType union type in the orchestration system. List every possible task type value and explain what the Task interface looks like.",
  },
  {
    id: 4,
    mode: "exploration",
  ...
  },
  {
    ...
    prompt:
      "How does the /api/orchestrate route handle authentication and session management? Trace the flow from when a POST request arrives to how errors are returned.",
  },
  {
    id: 6,
    mode: "exploration",
    shortName: "E2E Flow",
    prompt:
      "How does the code generation flow work end-to-end? Trace from user input in the ChatPanel through the generation pipeline to files being updated and previewed in the E2B sandbox. Cover all three generation modes: single, pipeline, and orchestrated.",
  },
  {
    id: 7,
    mode: "targeted",
    shortName: "Quality Gates",
    prompt:
      "Find the quality gates system in the orchestration pipeline. What types of gates exist, what are their categories, and how does the system determine if a generated project passes validation?",
  },
];
...
function buildWithSystemPrompt(task: TaskDef, toolBudget: number): string {
  ...
  if (task.id === 5) {
    lines.push(
      "Trace the full error handling chain including all HTTP status codes.",
    );
  }

  if (task.id === 6) {
    lines.push(
      "Do not end with a planning stub. Deliver the full end-to-end flow.",
    );
  }
...
function buildLazyLoadingMcpServerConfig(): Record<string, { command: string; args: string[] }> {
  ...
  const dbPath = path.join(TEST_DIR, ".lazyload/index.db");
...
// ---------------------------------------------------------------------------
// Phase 1: Setup - validate local test repo and index it
// ---------------------------------------------------------------------------

async function setup(): Promise<void> {
  if (!fs.existsSync(TEST_DIR)) {
    throw new Error(
      `Required test repository not found at ${TEST_DIR}. Expected benchmarks/.repos/test/ to exist.`,
    );
  }

  console.log("openhagen/test already present.");
  console.log("Indexing openhagen/test codebase...");
  const dbDir = path.join(TEST_DIR, ".lazyload");
...
function printConsoleTable(results: TaskResult[]): void {
  ...
  console.log("\n=== OPENHAGEN VALIDATION BENCHMARK ===");
  console.log(
...
function buildOutput(
  ...
  return {
    benchmark: options.benchmark ?? "openhagen-validation",
    timestamp: options.timestamp ?? new Date().toISOString(),
    target_codebase: options.target_codebase ?? "openhagen/test",
...
function writeJsonOutput(
  ...
  const filePath =
    ...
    path.join(
      RESULTS_DIR,
      `validation-test-${new Date().toISOString().split("T")[0]}.json`,
    );
...
function rescoreValidationFile(
  ...
  return writeJsonOutput(
    ...
    outputPath,
    {
      benchmark: parsed.benchmark ?? "openhagen-validation",
      target_codebase: parsed.target_codebase ?? "openhagen/test",
...
async function main(): Promise<void> {
  ...
  const overrideSummary = Object.entries(TOOL_BUDGET_OVERRIDES)
    .sort((a, b) => Number(a[0]) - Number(b[0]))
    .map(([taskId, budget]) => `${taskId}:${Math.min(MAX_TOOL_CALLS, budget)}`)
    .join(", ");

  console.log("=== OPENHAGEN VALIDATION BENCHMARK ===");
  console.log(`Target: openhagen/test | Model: ${MODEL}`);
  console.log(
  ...
  if (RUN_INSTALLED) {
    ...
    console.log(
      `FORCED mode settings: max-tool-calls=${MAX_TOOL_CALLS} (effective caps: targeted=${effectiveTargetedBudget}, exploration=${effectiveExplorationBudget}, overrides={${overrideSummary}}), format=${TOOL_RESPONSE_FORMAT}`,
    );
  }
  console.log(
